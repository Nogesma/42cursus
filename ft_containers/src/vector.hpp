...
namespace ft
{
	...
	public:
		...
		typedef typename std::iterator<pointer, vector> iterator;
		typedef typename std::iterator<const_pointer, vector> const_iterator;
	...
	//
	//	  for (size_type i = 0; i < _size; ++i)
	//		  _allocator.construct(_value + i, *(first + i));
	//  }
